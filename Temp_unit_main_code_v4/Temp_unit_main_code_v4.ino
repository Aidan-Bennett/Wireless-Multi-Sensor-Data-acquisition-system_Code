...
#define sensor_type 3 //Board id type. 1: ADC, 2: CO2 sensor, 3: Temp/RH, 4: --- (undefined)
...
/*=============== SHT35 temperature/relative humidity sensor variables =======*/
ClosedCube_SHT31D SHT35;
#define SHT35Addr 0x044
...
#define datasample_FRAM_byte_length 19 // max possible byte length assumed by the entire system is 46 bytes. last 1 byte for the checksum.
...
Data type layout does not include date/time as they should be all uint8_t, and also does not include checksum as it is always uint8_t and the last byte of the packet.
*/
const char data_type_layout[] = {'F', 'F', 'F', 'E'};  // three floats for the two temperature readings and one humidity reading.
...
void readSHT35() {
	SHT31D temp;
	temp = SHT35.readTempAndHumidity(SHT3XD_REPEATABILITY_HIGH, SHT3XD_MODE_CLOCK_STRETCH, 50); //timeout is 50 ms. not used with clock stretching mode
	temp_sample.SHT35_temperature = temp.t;
	temp_sample.SHT35_humidity = temp.rh;
	if (debug && checkUSBConnected()) {
		Serial.print(F(" (SHT35) Temp= "));
		Serial.print(temp_sample.SHT35_temperature);
		Serial.println(F(" C"));
		Serial.print(F(" (SHT35) RH= "));
		Serial.print(temp_sample.SHT35_humidity);
		Serial.println(F(" %"));
	}
...
void saveDataSampleToFRAM() {
	...
	while ((fram.readFloat( addr) != temp_sample.SHT35_temperature) ) {
		fram.writeFloat( addr, temp_sample.SHT35_temperature);
	}
	temp_sample.checksumValue += calcChecksum( temp_sample.SHT35_temperature);
	addr += sizeof(float);

	while ((fram.readFloat( addr) != temp_sample.SHT35_humidity) ) {
		fram.writeFloat( addr, temp_sample.SHT35_humidity);
	}
	temp_sample.checksumValue += calcChecksum( temp_sample.SHT35_humidity);
	addr += sizeof(float);
